...
public class PatientStaticEncounterPresenterTest extends ACUnitTestBaseRx {

    @Mock
    private PatientDashboardContract.ViewStaticEncounter viewStaticEncounter;
    @Mock
    ...
    private PatientStaticEncounterPresenter presenter;
    ...
    public void setUp() {
        ...
        presenter = new PatientStaticEncounterPresenter(patient, viewStaticEncounter, encounterDAO, visitApi);
    ...
    public void subscribe_allOk() {
        ...
        verify(viewStaticEncounter, times(2)).showStaticEncounter(encounter);
    }
    ...
    public void subscribe_nullEncounter() {
        ...
        presenter.subscribe();
        verify(encounterDAO, times(2)).getLastVitalsEncounter(patient.getUuid());
        verify(viewStaticEncounter, times(2)).showNoEncountersNotification();
    }

    @Test
    public void subscribe_errorResponse() {
        PowerMockito.when(NetworkUtils.isOnline()).thenReturn(true);
        when(restApi.getEncounters(anyString(), anyString(), anyString(), anyInt(), anyString())).thenReturn(mockErrorCall(401));
        Encounter encounter = new Encounter();
        when(encounterDAO.getLastVitalsEncounter(patient.getUuid())).thenReturn(Observable.just(encounter));
        presenter.subscribe();
        verify(viewStaticEncounter).showErrorToast(anyString());
        verify(encounterDAO).getLastVitalsEncounter(patient.getUuid());
        verify(viewStaticEncounter).showStaticEncounter(encounter);
    }
    ...
    public void subscribe_errorResponseNullEncounter() {
        ...
        verify(viewStaticEncounter).showErrorToast(anyString());
        verify(encounterDAO).getLastVitalsEncounter(patient.getUuid());
        verify(viewStaticEncounter).showNoEncountersNotification();
    }
    ...
    public void shouldStartFormDisplayActivityWithEncounter_allOK() {
        ...
        verify(viewStaticEncounter).startFormDisplayActivity(encounter);
    }
}
